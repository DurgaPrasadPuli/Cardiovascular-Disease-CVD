...
This prints the data-cleaning report, summary statistics, and both models' classification reports to the terminal, and writes all 9 EDA figures to `output/figures/`.
...
**What the diagrams show, in plain terms:**

- **Age** – The age box plot shows the `target = 0` group skews older (median 58 vs. 52) — the opposite of the original notebook's stated conclusion, which claimed older patients were more likely to be `target = 1`.
- **Sex** – The dataset is majority male (206 of 302), but proportionally, 75% of female patients are `target = 1` versus only 45% of male patients — sex splits the two groups very differently once you control for the group sizes.
- **Resting blood pressure & cholesterol** – Both are higher on average in the `target = 0` group (BP: 134 vs. 129 mmHg; cholesterol: 251 vs. 243 mg/dL), though the `target = 1` group has a slightly wider spread and higher maximum cholesterol values — neither is a clean-cut predictor on its own.
- **Maximum heart rate (thalach)** – This is one of the strongest single-variable signals: the `target = 1` group reaches a noticeably higher mean peak heart rate (158 vs. 139 bpm) — again the reverse of the original notebook's conclusion.
- **Exercise-induced angina & ST slope** – Angina during exercise is far more common in the `target = 0` group (55% vs. 14%), and a flat/downsloping ST segment likewise concentrates in that group.
- **Major vessels (ca) & thalassemia (thal)** – 79% of the `target = 1` group has zero vessels colored by fluoroscopy, versus only 33% of the `target = 0` group; thalassemia code 2 dominates the `target = 1` group (79%) while code 3 dominates `target = 0` (65%).
